...
\begin{center}
  ...
  % Still rough title
  {\LARGE A Program Logic for Verification of Security Properties in Secure
  ECMAScript} \\[1cm]
...
\section{Separation Logic}
  ...
  \TODO{Make heap cells single value}
  \TODO{diagram of (x -> 4, y) * (y -> 3, x)}
...
\chapter{SES Language Syntax \& Operational Semantics}
  ...
  \TODO{Include/discuss my new axioms}
  \TODO{Discuss methods for obtaining details of semantics for new functionality
  of semantics for new/existing operations}
...
\TODO{Need a more concrete example of how this could come about. I'm still not
  entirely happy with this -- I suspect it comes from a lack of familiarity with
  how pure assertions behave within Separation Logic (but I guess a part of this
  project is to figure out exactly that!...)}
\gds{here you go}
...
creation, where splitting out to two rules just isn't possible -- at one point
we had 4 variations of this rule, before fixing on the language.
...
Initialise bps for 2 or 3 objects, function object ($L_2$), prototype object
($L_1$), and named function scope oject ($L_3$)
Also $\lop$ and $\lfp$ updated.

(Function call) Nothing much interesting here, 
(Object construction) Write the rule!
Bp for this object, and for object passed as parameter.
Bp created for new activation record.

(This) Unchanged.

(Restricted evaluation) New definition, but of itself has no bp assertions, they
may be passed through to code executing within, though. Key effect is
redefinition of $\ls$. We may be able to drop bold g.
