...
program will execute, their use for practical program verification is limited by
a lack of expressivity and compositionality that would be achieved with use of a
logic-based set of axioms.
\TODO{My background is lacking here, (or I've forgotten it) -- discuss}

We first extend separation logic to be able to express reachability over
pointers in the heap, and then produce a Hoare-triple axiom for each expression
in the syntax of the language.

\section{Extending Separation Logic}
We are aiming to reason about reachability through pointer accesses in the heap.
Separation logic is inherently unable to reason about these since the
information required to satisfy such assertions is inherently framed off to
the part of the assertion that deals with the operational aspects of the command.

We extend the logic to support this by reintroducing a global heap state to the
assertion satisfaction relation and redefine the existing logical operators to
carry around the global heap state without examining it.
