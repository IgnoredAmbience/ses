...
\section{JavaScript}
  ...
  * Flattening of statement/expression structure - dictated by syntax, assumes
  all programs parse as valid ES, not big semantic impacts (ES syntax extremely
  complex, eg ; insertion)

  * Automatic type casting assumed, not modelled, would be very important in a
  full logic, for now we skirt the issue by just enforcing that the primitive
  type is supplied in the preconditions where required.
  ...
  is recursively checked for the field. This operation is performed by the
  $\proto$ predicate in the Operational Semantics and Program Logic. The list of
  ...
  % both the above examples need diagrams
