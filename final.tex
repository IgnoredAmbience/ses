...
Consider $x \pointsto 1 \sep (x \pointsto 2 \wand Q)$
...
Derivation for a naive definition of $\wand$ is unsound:
\begin{align*}
...
sub-assertions. The local portion of the heap should always be a subset of the
global portion.

To enforce this property, whilst maintaining the right-adjoint, we require it is
for satisfaction of the $\wand$ operator.

However, this prohibits us from using $\wand$ for hypothetical heap update
operations, we hence need to introduce this to support this second use.

The new operator to be defined is the \emph{box-wand}, $\boxwand$. In addition
