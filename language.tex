...
var Membrane = function(ref) {
  ...
  var MembraneRef = function MembraneRef(r) {
    var access = function(field) {
      if(!killed) return MembraneRef(r[field]);
  ...
  return {
    access: MembraneRef(ref),
    kill:   function() { killed = true; }
  };
}
\end{verbatim}

For a protected object $L$, protected by a membrane instance object $M$, there
will be some set $S$ of pointers to $L$.
\[
  Protected(L, S, M) = L \bp S \cup \{(M,\js{ref})\} \sep \rv \not\doteq L
\]
Calls to functions of the membrane must maintain this predicate.
