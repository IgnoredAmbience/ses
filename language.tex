...
var Membrane = function(ref) {
  ...
  var MembraneRef = function(ref) {
    var access = function(field) {
      ...
      else throw("Membrane has been killed");
    }
...
may not be returned.
\[
  Protected(L, S, M) = L \bp S \cup \{(M,\js{ref})\} \sep \rv \not\doteq L
\]

We show that $Protected(L,S,M)$ holds after a call to \js{Membrane(L)}, and
throughout a call to the accessor function, the kill function is omitted as the
proof is trivial.
...
Idea: collect newly created MRs in a set that is propagated with the safety
predicate.
...
\begin{figure}
  \[
      ...
      \tjs{var access = function(field) \{} \\
        \indblock{
          ...
          \tjs{\} else \{} \\
            \indblock{
              \logic{\ltrue}
              \tjs{throw("Membrane has been killed");}
              \logic{\lfalse}
            }
          \tjs{\}} \\
          \logic{}
    ...
    \end{array}
  \]
  \caption{Proof outline for \js{Membrane} and its \emph{nested} \js{MembraneRef}
    and \js{access} functions}
\end{figure}
