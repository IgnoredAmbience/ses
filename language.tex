...
\newcommand{\editlabel}[3]{~\vg\nphantom{#1:}\colorbox{#2}{\textbf{#1:}}\tabfill{\textrm{ #3}}\\}
  ...
  \[
    \begin{array}{l}
      ...
      \logic{
        ...
        MR(M, r, F_A, S, T) \sep
        MRset([F_A],T) \sep
        \obj_L(\js{access:}F_A\js{,kill:}F_K) \sep
        \rv \doteq L
      } \\
      \\
      \logic{
        \js{field} \dotin Str \sep
        MR(M,r,F_A1,S_1,T_1) \sep
        MRset(Ls,T_1) \sep
        alive(M) \sep
        (r, \js{field}) \pointsto V \sep
        V \dotin \loc \sep
        V \bp S_2
      } \\
      \js{access(field)} \\
      \logic{
        \js{field} \dotin Str \sep
        MR(M,r,F_A1,S_1,T_2) \sep
        MR(M,r,F_A2,S_2,T_2) \sep
        MRset(F_A2:Ls,T_2) \sep
        alive(M) \sep
        (r, \js{field}) \pointsto V \sep
        V \dotin \loc \sep
        V \bp S_2
      ...
      \\
      \logic{
        kill(F_K,M) \sep (alive(M) \lor dead(M))
      } \\
      \js{kill()} \\
      \logic{
        kill(F_K,M) \sep dead(M)
      }
    \end{array}
  \]
  \caption{Desired specification for a Membrane}
\end{figure}

Notation:
\begin{itemize}
  \item $MR(M,r,F_A,S,T)$ \qquad Membrane reference for Membrane $M$ protecting object
$r$, corresponding access function $F_A$, (safe-side) backpointer set $S$ for
$r$, membrane-constructed backpointer set $T$.
\[ MR(M,r,F_A,S,T) = \]

  \item $MRset(Ls,T)$ \qquad Membrane-backpointer constructor. Given a list of
constructed $MR$s (access functions thereon?), provide (maximal) backpointer set
for protected references for the Membrane.
\[ MRset([], \{\}) = \]
\[ MRset(L:Ls, T \cup Ts) = MRset(Ls, Ts) \sep \]

  \item $alive(M)$ and $dead(M)$ \qquad encapsulates Membrane data object (AR),
    on which is contained initial reference to $r$, the killswitch, (kill
    function), MembraneRef constructor. (Use $membraneInstance$?)
\[ MembraneInstance(M) = \]
\[ alive(M) = MembraneInstance(M) \sep (M, killed) \pointsto \false \]
\[ dead(M) = MembraneInstance(M) \sep (M, killed) \pointsto \true \]

  \item $kill(F_K, M)$ \qquad Represents the kill function $F_K$ for membrane $M$.
\[ kill(F_K, M) = \newfun_{F_K}(M:\_, \_ , \lambda_K, \_) \]
...
\begin{figure}[b]
  \[
    \begin{array}{l}
      \tjs{var Membrane = function(ref) \{} \\
        \indblock{
          \logic{
            \fullobj_M \args{
              \js{ref}: L, @this: \_, @proto: \nil, \js{killed}: \und, \\
              \js{MembraneRef}: \und, \js{access}: \und, \js{kill}: \und
            } \sep {} \\
            Protected(L,S,M) \sep \ls \doteq M:Ls
          } \\
          \tjs{var killed = false;} \\
          ...
          \logic{
            \fullobj_M \args{
              \js{ref}: L, @this: \_, @proto: \nil, \js{killed}: \false, \\
              \js{MembraneRef}: F_{MR}, \js{access}: \und, \js{kill}: \und
            } \sep {} \\
            Protected(L,S,M) \sep \newfun_{F_{MR}}(\ls,\js{ref},\lambda_{MR},\_)
              \sep \ls \doteq M:Ls
          } \\
          \tjs{var access = MembraneRef(ref);} \\
          \logic{
            \fullobj_{MR_0}(\js{ref}: L, @this: \_, @proto: \nil, \js{access}:
              F_{A_0}) \sep {} \\
            \newfun_{F_{A_0}}(MR_0:\ls, \js{field}, \lambda_A, \_) \sep {} \\
            ((L \dotin\uvals \sep \rv \doteq L) \lor (L \notdotin\uvals \sep \rv
            \doteq F_{A_n}))
          } \\
    ...
    \end{array}
  \]
  \caption{Proof \emph{outline} for \js{Membrane}}
\end{figure}
  ...
  \[
    \begin{array}{l}
      \tjs{var MembraneRef = function(ref) \{} \\
        \indblock{
          \logic{
            \fullobj_{MR_n}(\js{ref}: L, @this: \_, @proto: \nil, \js{access}:
              \und) \sep {} \\
            \bpGen(L, s, s', \{(MR_n, \js{ref})\}) \sep
            \ls \doteq MR_n : A_{n-1} : MR_{n-1} : ... : A_0 : MR_0 : M : Ls
          } \\
          \tjs{var access = function(field) \{...\}} \\
          \logic{
            \fullobj_{MR_n}(\js{ref}: L, @this: \_, @proto: \nil, \js{access}:
              F_{A_n}) \sep {} \\
            \newfun_{F_{A_n}}(\ls, \js{field}, \lambda_A, \_) \sep {} \\
            \ls \doteq MR_n : A_{n-1} : MR_{n-1} : ... : A_0 : MR_0 : M : Ls
          } \\
          \tjs{if(primitive(ref)) \{} \\
            \indblock{
              \logic{(MR_n, \js{ref}) \pointsto L \sep \ls \doteq MR_n:...}\\
              \tjs{return ref;} \\
              \logic{\rv \doteq L}
            }
          \tjs{\} else \{} \\
            \indblock{
              \logic{(MR_n, \js{access}) \pointsto F_{A_n} \sep \ls \doteq
              MR_n:...    } \\
              \tjs{return access;} \\
              \logic{\rv \doteq F_{A_n}}
            }
          \tjs{\}} \\
          \logic{
            \fullobj_{MR_n}(\js{ref}: L, @this: \_, @proto: \nil, \js{access}:
      ...
      \tjs{var access = function(field) \{} \\
        \indblock{
          \logic{
            \ls \doteq A_n : MR_n : ... : A_0 : MR_0 : M : Ls \sep {} \\
            \bigsep_{i \in 0..n}((A_i,\js{killed}) \pointsto \none \sep
              (MR_i,\js{killed}) \pointsto \none) \sep (M, \js{killed}) \pointsto
              B \sep {} \\
              \istrue(B)
          } \\
          \tjs{if(!killed) \{} \\
            \indblock{
              \logic{
                \ls \doteq A_n : MR_n : ... : A_0 : MR_0 : M : Ls \sep {} \\
                (A_n, \js{ref}) \pointsto \none \sep (MR_n, \js{ref}) \pointsto
                  L \sep (A_n, \js{field}) \pointsto V \sep \getValue(\_,V,X) \sep X
                  \dotin \uvars \sep {} \\
                \getValue(\_, \fld{L}{X}, L') \sep \bpGen(L',s)
               % \bigsep_{i \in 0..n}((A_i,\js{MembraneRef}) \pointsto \none \sep
               %   (MR_i,\js{MembraneRef}) \pointsto \none) \sep {} \\
               % (M, \js{MembraneRef}) \pointsto F_{MR}
              } \\
              ...
              \logic{
                ...
                (A_n, \js{ref}) \pointsto \none \sep (MR_n, \js{ref}) \pointsto
                  L \sep (A_n, \js{field}) \pointsto V \sep \getValue(\_,V,X) \sep X
                  \dotin \uvars \sep {} \\
                \getValue(\_, \fld{L}{X}, L') \sep \bpGen(L',s, s',
                  \{(MR_{n+1}, \js{ref})\}) \sep {} \\
                \fullobj_{MR_{n+1}}(\js{ref}: L', @this: \_, @proto: \nil, \js{access}:
                  F_{A_{n+1}}) \sep {} \\
                \newfun_{F_{A_n}}(MR_{n+1}:\ls, \js{field}, \lambda_A, \_) \sep {} \\
                ((L' \dotin\uvals \sep \rv \doteq L') \lor (L' \notdotin\uvals \sep \rv
                \doteq F_{A_{n+1}}))
              } \\
            }
