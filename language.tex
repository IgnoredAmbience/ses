...
\begin{display}{Assertion satisfaction}
  $\begin{array}{lcl}
    ...
    h,h_g,L,\env \satisfies P \wand Q & \iff & \forall h' \st
      (h',h_g[h'],L,\env \satisfies P) \land h \disj h' \\
...
heap, we cannot keep both the property $P \sep Q \vdash R \iff P \vdash Q \wand
R$ and maintain use of $\wand$ as the weakest precondition for a command.

It is possible to introduce another semantics for $\wand$, where the global heap
is not made consistent with the new portion of heap, this would maintain the
first property, but break its use for weakest precondition. (sketch proof in
handwritten notes).
