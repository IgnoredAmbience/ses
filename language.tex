...
\[
  \begin{array}{l}
    ...
    \logic{
      ...
      % fun pre
      \FIXME{We need $A, A_p$ before we create them?}
      \exists A, A_p \st \lop \bp s_3 \cup \{(A_p, @proto)\} \sep \lfp \bp s_4
        \cup \{(A, @proto)\} \sep {} \\
      \forall l' \in \elem(\ls) \st \exists s' \st
        l' \bp s' \cup \{(A,@scope)\} \sep {} \\
      % assign
      (L,@frozen) \pointsto \false
    } \\
    ...
    \logic{
      ...
      % fun pre
      \exists K, K_p \st \lop \bp s_5 \cup \{(K_p, @proto)\} \sep \lfp \bp s_6
        \cup \{(K, @proto)\} \sep {} \\
      \forall l' \in \elem(\ls) \st \exists s' \st
