...
\begin{display}{Auxiliary Predicates}
  ...
  \jaxiom{\scopePtrs(Ls, s, s', n)}{\bigsep_{0\leq i<\length(Ls)}(
  ...
  \staterule{(Function)}
  {
    P = \lop \bp s_1 \sep \lfp \bp s_2 \sep \bigsep_{0\leq i<\length(\ls)} \lstitem(i,\ls) \bp s'_i \\
      Q = \exists L_1,L_2 \st \left(\begin{array}{l}
        ...
        % Other bits
        \rv \doteq L_2 \sep
        \bigsep_{0\leq i<\length(\ls)} \lstitem(i,\ls) \bp \{(L_2,@scope)\} \cup s'_i
    \end{array}\right)
  }
  {\tr P {\jsfun{x}{e}} Q}
  \vg
...
\gds{Want to fix this by doing $\scopePtrs(blah)\sepish L\bp\{\}$ ? Since the bp
assertion is exact, you can use wand to get a fairly tight frame
like so:
$\scopePtrs(blah)\sepish L\bp\{\} \equiv L\bp\{\}\sep (L\bp\{\} \wand
\scopePtrs(blah)\sepish L\bp\{\})$
Note that we're not speculating different values, just shifting our footprint
around. So the regular wand will do.}
...
\gds{On my system the ``var protected = ref'' and similar mysteriously lose
  whitespace between the ``var'' and the ``protected''.}
