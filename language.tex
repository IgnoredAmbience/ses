...
\end{display}
  ...
  \begin{display}{Local Variable Declarations}
    \jaxiom{\defs(\js{x},l,\jsvar{y})}{(l,y) \pointsto \und \quad$ \= iff
      $\js{x} \neq \js{y}} \\
  ...
  \begin{display}{Heap Update $H[H']$}
    \jaxiom{H[\emp]}{H} \\
    \jaxiom{H[(l,x) \pointsto v]}{H \disju (l,x) \pointsto v \qquad \mbox{iff }
      (l,x) \not\in \dom(H)} \\
...
\TODO{This appears to contradict the program logic, was the decision to shadow
  imports?}
